...
public class Main {
    ...
    private void setDefaults() {
        ...
        canvasSize = new Dimension(SettingsCell.xCell * this.totalSize + this.gapSize + this.borderSize * 2 + 150,
                SettingsCell.yCell * this.totalSize + this.gapSize + this.borderSize * 2 + 50);

        handlerObjects.RemoveAll(this);

        this.setMap();
        handlerCell.updateCells(this);

        int y = 50;
        for (Player dP : Player.values())
            if (dP != Player.Dead) {
            ...
            if (mapData != null) {
                for (int x = 0; x < SettingsCell.xCell; x++) {
                    for (int y = 0; y < SettingsCell.yCell; y++) {
                        ...
                        Cell c = TypeCellPart.createCell(typeCell, this.borderSize + this.gapSize + x * this.totalSize,
                                this.borderSize + this.gapSize + y * this.totalSize + 10, x, y);
    ...
    public void undoState() {
        if (handlerTick.pause || isSimulating())
            return;
